...
```json
{
  ...
  "keywords": ["npm", "package-lock", "semver", "range", "calculator", "cli", "engines", "node-version"],
...
test('two overlapping ranges returns intersection', () => {
  const result = computeIntersection([entry('a', '>=14.0.0'), entry('b', '>=18.0.0')])
  assert.ok(result.intersection !== null)
  assert.equal(result.conflicts.length, 0)
...
test('parses v3 lockfile and returns all packages', () => {
  ...
  assert.ok(packages.length >= 3, `expected >=3 packages, got ${packages.length}`)
...
test('parses v1 lockfile format', () => {
  const content = readFileSync(join(fixturesDir, 'package-lock-v1.json'), 'utf-8')
  const packages = parseNpmLockfile(content)
...
test('handles scoped packages', () => {
  const content = ['# yarn lockfile v1', '', '"@babel/core@^7.0.0":', '  version "7.24.0"', '  resolved "https://registry.npmjs.org/@babel/core/-/core-7.24.0.tgz#abc"', '  integrity sha512-abc', ''].join('\n')
  const packages = parseYarnClassicLockfile(content)
...
test('handles scoped packages', () => {
  const content = ['__metadata:', '  version: 6', '  cacheKey: 8', '', '"@babel/core@npm:^7.0.0":', '  version: 7.24.0', '  resolution: "@babel/core@npm:7.24.0"', '  checksum: abc', '  languageName: node', '  linkType: hard', ''].join('\n')
  const packages = parseYarnBerryLockfile(content)
  const babelCore = packages.find(p => p.name === '@babel/core')
  assert.ok(babelCore, '@babel/core not found')
  assert.equal(babelCore!.version, '7.24.0')
...
function nameAndVersionFromKey(key: string): { name: string; version: string } | null {
...
test('detectYarnVersion distinguishes classic and berry', () => {
  assert.equal(detectYarnVersion('# yarn lockfile v1\n\nsemver@^7:\n  version "7.6.3"\n'), 'yarn-classic')
...
export function resolveLocal(packages: Package[], nodeModulesDir: string): Package[] {
  return packages.map(pkg => {
    const pkgJsonPath = join(nodeModulesDir, pkg.name, 'package.json')
    if (!existsSync(pkgJsonPath)) return pkg

    try {
      const pkgJson = JSON.parse(readFileSync(pkgJsonPath, 'utf-8')) as LocalPackageJson
      return {
        ...pkg,
...
test('skips all fetches when offline is true', async () => {
  ...
  const input: Package[] = [{ name: 'semver', version: '7.6.3', engines: { node: '>=10' } }]
...
async function fetchManifest(name: string, version: string, cache?: Cache): Promise<RegistryManifest | null> {
...
export async function resolveRegistry(packages: Package[], opts: ResolveOptions = {}): Promise<Package[]> {
  ...
  for (let i = 0; i < packages.length; i += concurrency) {
    ...
    const resolved = await Promise.all(
      batch.map(async pkg => {
        const [current, latest] = await Promise.all([fetchManifest(pkg.name, pkg.version, opts.cache), fetchManifest(pkg.name, 'latest', opts.cache)])
...
function collectRanges(packages: Package[], engineKey: string, useLatest: boolean): RangeEntry[] {
...
export function analyzeEngines(packages: Package[], manager: ManagerType): AnalysisTarget[] {
  ...
  return engineTargets.map(target => {
    ...
    const { intersection: latestIntersection, conflicts: latestConflicts } = computeIntersection(latestRanges)
...
import { join, dirname } from 'node:path'
import { fileURLToPath } from 'node:url'
import { analyzePeers, detectPeerTargets, WELL_KNOWN_PEERS } from '../../src/analyzer/peers.js'
import type { Package } from '../../src/types.js'
...
export const WELL_KNOWN_PEERS = ['typescript', 'react', 'react-dom', 'vue', 'svelte', 'webpack', 'vite', 'rollup', 'esbuild', 'jest', 'vitest', 'next', 'nuxt', 'astro'] as const
...
export function detectPeerTargets(projectDir: string, extraChecks: string[] = []): string[] {
  try {
    ...
    const detected = (WELL_KNOWN_PEERS as readonly string[]).filter(p => p in allDeps)
...
function collectRanges(packages: Package[], target: string, useLatest: boolean): RangeEntry[] {
  const ranges: RangeEntry[] = []
  for (const pkg of packages) {
...
export function analyzePeers(packages: Package[], targets: string[]): AnalysisTarget[] {
...
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { renderTargets, renderJson, renderHeader } from '../../src/output/table.js'
...
test('renderTargets shows conflict package when conflicts exist', () => {
  const withConflicts: AnalysisTarget[] = [
    {
      ...targets[0],
      latestConflicts: [{ package: 'old-pkg', version: '1.0.0', range: '<12.0.0' }],
...
function packageRow(ui: ReturnType<typeof cliui>, entry: RangeEntry, latestVersion: string, prefix = '  '): void {
  ui.div({ text: prefix + entry.package, width: COL.pkg }, { text: entry.version, width: COL.installed }, { text: latestVersion, width: COL.latest }, { text: entry.range, width: COL.range })
}
...
function renderTarget(target: AnalysisTarget, opts: RenderOptions): string {
  ...
  // Summary
  ui.div({ text: '  Safe range (installed):', width: 30 }, { text: target.intersection ?? '⚠  no compatible range', width: WIDTH - 30 })
  ui.div(
  ...
  if (target.ranges.length > 0 || opts.all) {
    ui.div('')
    // Column headers
    ui.div({ text: '  Package', width: COL.pkg }, { text: 'Installed', width: COL.installed }, { text: 'Latest', width: COL.latest }, { text: 'Range (installed)', width: COL.range })

    for (const entry of target.ranges) {
      const matchingLatest = target.latestRanges.find(r => r.package === entry.package)
...
export function renderTargets(targets: AnalysisTarget[], opts: RenderOptions): string {
  const active = targets.filter(t => t.ranges.length > 0 || opts.all)
  return active.map(t => renderTarget(t, opts)).join(`\n${SEPARATOR}\n\n`)
}

export function renderHeader(lockfilePath: string, manager: string, targetNames: string[], totalPackages: number): string {
  const ui = cliui({ width: WIDTH })
